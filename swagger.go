...
func WrapHandler(h *webdav.Handler) macaron.Handler {
	t := template.New("swagger.index.html")
	...
	return func(ctx *macaron.Context){
		var matches []string
		if matches = re.FindStringSubmatch(ctx.Req.RequestURI); len(matches) != 3{
		...
		case "index.html":
			...
			index.Execute(ctx.Resp, s)
		case "doc.json":
			doc, _ := ReadDoc()
			ctx.Resp.Write([]byte(doc))
			return
		default:
